...
all_output_individual[, drug := drug_specific]
all_output_individual[, bug := bug_specific]

saveRDS(all_output_individual, file=paste0("output/", bug_specific, "_", drug_specific, "ind.rds"))
...
Models_fixed[, bug := bug_specific]
Models_fixed[, drug := drug_specific]
saveRDS(Models_fixed, file=paste0("output/", bug_specific, "_", drug_specific, "_fixed.rds"))
...
### want to do the same for the random effects ###
  ...
  Models_random[, bug := bug_specific]
  Models_random[, drug := drug_specific]
  saveRDS(Models_random, file=paste0("output/", bug_specific, "_", drug_specific, "_random.rds"))
...
var_combo[, bug := bug_specific]
var_combo[, drug := drug_specific]
saveRDS(var_combo, file=paste0("output/", bug_specific, "_", drug_specific, "_var.rds"))
